import { Button, Dialog, DialogPanel, DialogTitle } from "@headlessui/react";
import { useState } from "react";
import { useForm } from "react-hook-form"
import { yupResolver } from "@hookform/resolvers/yup"
import * as yup from "yup"
import { toast } from 'react-toastify'

import api from "../../api-intercepter";
import { AppData } from "../../context/AppContext";
import { PencilIcon, PlusIcon } from "@heroicons/react/24/outline";

const schemaAdd = yup
    .object({
        title: yup.string().required(),
        description: yup.string().required(),
        target_value: yup
            .number()
            .transform((value, originalValue) =>
                originalValue === "" ? undefined : value
            )
            .required("Target value is required"),
        actual_value: yup
            .number()
            .transform((value, originalValue) =>
                originalValue === "" ? undefined : value
            )
            .nullable(),
        status: yup.string().required(),
        assigned_user: yup.string().required(),
        start_date: yup
            .string()
            .transform((value, originalValue) =>
                originalValue ? new Date(originalValue).toISOString() : null
            )
            .required(),

        end_date: yup
            .string()
            .transform((value, originalValue) =>
                originalValue ? new Date(originalValue).toISOString() : null
            )
            .required(),
    })

const schemaEdit = yup
    .object({
        title: yup.string().required(),
        description: yup.string().required(),
        target_value: yup
            .number()
            .transform((value, originalValue) =>
                originalValue === "" ? undefined : value
            )
            .required("Target value is required"),
        actual_value: yup
            .number()
            .transform((value, originalValue) =>
                originalValue === "" ? undefined : value
            )
            .nullable(),
        status: yup.string().required(),
        assigned_user: yup.string().required(),
        start_date: yup
            .string()
            .transform((value, originalValue) =>
                originalValue ? new Date(originalValue).toISOString() : null
            )
            .required(),
        end_date: yup
            .string()
            .transform((value, originalValue) =>
                originalValue ? new Date(originalValue).toISOString() : null
            )
            .required(),
    })

const statusList = [
    { id: "1", name: "On Track" },
    { id: "2", name: "At Risk" },
    { id: "3", name: "Off Track" },
];

export default function KPIAddEditModal({ action, id = "", data }) {

    const schema = action === "Add" ? schemaAdd : schemaEdit;

    const [isOpen, setIsOpen] = useState(false);
    const { userList, fetchKpiList } = AppData()

    const {
        register,
        handleSubmit,
        formState: { errors },
        reset,
    } = useForm({
        resolver: yupResolver(schema),
    })

    function open() {
        setIsOpen(true);

        if (action === "Edit") {
            reset({
                ...data,
                assigned_user: data.assigned_user?._id || "",
                start_date: data.start_date ? data.start_date.slice(0, 10) : "",
                end_date: data.end_date ? data.end_date.slice(0, 10) : "",
            });
        }
    }

    function close() {
        setIsOpen(false);

        reset()
    }

    const onSubmit = async (data) => {
        try {
            let response;
            if (action === "Add") {
                response = await api.post(`/api/v1/kpis`, data);
            } else if (action === "Edit") {
                response = await api.put(`/api/v1/kpis/${id}`, data);
            }

            if (response) {
                toast.success(response?.data?.message);
            }

            close()
            fetchKpiList()
            reset()
        } catch (error) {
            const message = error.response?.data?.message || "Something went wrong";
            toast.error(message);
        }
    }

    return (
        <>
            <Button
                onClick={open}
                className="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white 
                   hover:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-400"
            >

                {action === "Add" ? (<PlusIcon width={15} />) : <PencilIcon width={15} />}
            </Button>

            {/* Add  */}
            {action === "Add" && (
                <Dialog open={isOpen} className="relative" onClose={close}>
                    <div className="fixed inset-0 " />
                    <div className="fixed inset-0 z-10 w-screen ">
                        <div className="flex min-h-full items-center justify-center p-4">
                            <DialogPanel className="w-full max-w-lg rounded-xl bg-white p-6 shadow-md">
                                <DialogTitle
                                    as="h3"
                                    className="text-lg font-semibold leading-6 text-gray-900 border-b"
                                >
                                    Add KPI
                                </DialogTitle>

                                <form onSubmit={handleSubmit(onSubmit)} className="mt-4 space-y-4">
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700">
                                            Title
                                        </label>
                                        <input
                                            type="text"
                                            name="title"
                                            {...register("title")}
                                            className="block w-full rounded-md border-gray-300 px-3 py-1.5 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                        />
                                        <p className='text-red-500'>{errors.title?.message}</p>
                                    </div>

                                    <div>
                                        <label className="block text-sm font-medium text-gray-700">
                                            Description
                                        </label>
                                        <input
                                            type="text"
                                            name="description"
                                            autoComplete=""
                                            {...register("description")}
                                            className="block w-full rounded-md border-gray-300 px-3 py-1.5 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                        />
                                        <p className='text-red-500'>{errors.description?.message}</p>
                                    </div>


                                    <div>
                                        <label className="block text-sm font-medium text-gray-700">
                                            Target Value
                                        </label>
                                        <input
                                            type="number"
                                            name="target_value"
                                            {...register("target_value")}
                                            className="block w-full rounded-md border-gray-300 px-3 py-1.5 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                        />
                                        <p className='text-red-500'>{errors.target_value?.message}</p>
                                    </div>

                                    <div>
                                        <label className="block text-sm font-medium text-gray-700">
                                            Actual Value
                                        </label>
                                        <input
                                            type="number"
                                            name="actual_value"
                                            autoComplete=""
                                            value={0}
                                            {...register("actual_value")}
                                            className="block w-full rounded-md border-gray-300 px-3 py-1.5 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                        />
                                        <p className='text-red-500'>{errors.actual_value?.message}</p>
                                    </div>

                                    <div>
                                        <label className="block text-sm font-medium text-gray-700">
                                            Assigned
                                        </label>
                                        <select
                                            name="assigned_user"
                                            {...register("assigned_user")}
                                            className="block w-full rounded-md border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                        >
                                            <option value="">Select a user</option>
                                            {userList && userList.map((u) => (
                                                <option key={u._id} value={u._id}>
                                                    {u.username}
                                                </option>
                                            ))}
                                        </select>
                                        <p className='text-red-500'>{errors.assigned_user?.message}</p>
                                    </div>

                                    <div>
                                        <label className="block text-sm font-medium text-gray-700">
                                            Status
                                        </label>
                                        <select
                                            name="status"
                                            {...register("status")}
                                            className="block w-full rounded-md border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                        >
                                            <option value="">Select a Status</option>
                                            {statusList && statusList.map((s) => (
                                                <option key={s.id} value={s.name}>
                                                    {s.name}
                                                </option>
                                            ))}
                                        </select>
                                        <p className='text-red-500'>{errors.status?.message}</p>
                                    </div>


                                    <div>
                                        <label className="block text-sm font-medium text-gray-700">
                                            Start Date
                                        </label>
                                        <input
                                            type="date"
                                            name="start_date"
                                            {...register("start_date")}
                                            className="block w-full rounded-md border-gray-300 px-3 py-1.5 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                        />
                                        <p className='text-red-500'>{errors.start_date?.message}</p>
                                    </div>

                                    <div>
                                        <label className="block text-sm font-medium text-gray-700">
                                            End Date
                                        </label>
                                        <input
                                            type="date"
                                            name="end_date"
                                            {...register("end_date")}
                                            className="block w-full rounded-md border-gray-300 px-3 py-1.5 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                        />
                                        <p className='text-red-500'>{errors.end_date?.message}</p>
                                    </div>


                                    <div className="mt-6 flex justify-end gap-2">
                                        <Button
                                            type="button"
                                            onClick={close}
                                            className="rounded-md bg-gray-200 px-4 py-2 text-sm font-medium text-gray-800 hover:bg-gray-300 focus:outline-none"
                                        >
                                            Cancel
                                        </Button>
                                        <Button
                                            type="submit"
                                            className="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-400"
                                        >
                                            Save
                                        </Button>
                                    </div>
                                </form>
                            </DialogPanel>
                        </div>
                    </div>
                </Dialog>
            )}


            {/* Edit  */}
            {action === "Edit" && (
                <Dialog open={isOpen} className="relative" onClose={close}>
                    <div className="fixed inset-0 " />
                    <div className="fixed inset-0 z-10 w-screen ">
                        <div className="flex min-h-full items-center justify-center p-4">
                            <DialogPanel className="w-full max-w-lg rounded-xl bg-white p-6 shadow-md">
                                <DialogTitle
                                    as="h3"
                                    className="text-lg font-semibold leading-6 text-gray-900 border-b"
                                >
                                    Edit KPI
                                </DialogTitle>

                                <form onSubmit={handleSubmit(onSubmit)} className="mt-4 space-y-4">
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700">
                                            Title
                                        </label>
                                        <input
                                            type="text"
                                            name="title"
                                            {...register("title")}
                                            className="block w-full rounded-md border-gray-300 px-3 py-1.5 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                        />
                                        <p className='text-red-500'>{errors.title?.message}</p>
                                    </div>

                                    <div>
                                        <label className="block text-sm font-medium text-gray-700">
                                            Description
                                        </label>
                                        <input
                                            type="text"
                                            name="description"
                                            autoComplete=""
                                            {...register("description")}
                                            className="block w-full rounded-md border-gray-300 px-3 py-1.5 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                        />
                                        <p className='text-red-500'>{errors.description?.message}</p>
                                    </div>


                                    <div>
                                        <label className="block text-sm font-medium text-gray-700">
                                            Target Value
                                        </label>
                                        <input
                                            type="number"
                                            name="target_value"
                                            {...register("target_value")}
                                            className="block w-full rounded-md border-gray-300 px-3 py-1.5 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                        />
                                        <p className='text-red-500'>{errors.target_value?.message}</p>
                                    </div>

                                    <div>
                                        <label className="block text-sm font-medium text-gray-700">
                                            Actual Value
                                        </label>
                                        <input
                                            type="number"
                                            name="actual_value"
                                            autoComplete=""
                                            value={0}
                                            {...register("actual_value")}
                                            className="block w-full rounded-md border-gray-300 px-3 py-1.5 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                        />
                                        <p className='text-red-500'>{errors.actual_value?.message}</p>
                                    </div>

                                    <div>
                                        <label className="block text-sm font-medium text-gray-700">
                                            Assigned
                                        </label>
                                        <select
                                            name="assigned_user"
                                            {...register("assigned_user")}
                                            className="block w-full rounded-md border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                        >
                                            <option value="">Select a user</option>
                                            {userList && userList.map((u) => (
                                                <option key={u._id} value={u._id}>
                                                    {u.username}
                                                </option>
                                            ))}
                                        </select>
                                        <p className='text-red-500'>{errors.assigned_user?.message}</p>
                                    </div>

                                    <div>
                                        <label className="block text-sm font-medium text-gray-700">
                                            Status
                                        </label>
                                        <select
                                            name="status"
                                            {...register("status")}
                                            className="block w-full rounded-md border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                        >
                                            <option value="">Select a Status</option>
                                            {statusList && statusList.map((s) => (
                                                <option key={s.id} value={s.name}>
                                                    {s.name}
                                                </option>
                                            ))}
                                        </select>
                                        <p className='text-red-500'>{errors.status?.message}</p>
                                    </div>


                                    <div>
                                        <label className="block text-sm font-medium text-gray-700">
                                            Start Date
                                        </label>
                                        <input
                                            type="date"
                                            name="start_date"
                                            {...register("start_date")}
                                            className="block w-full rounded-md border-gray-300 px-3 py-1.5 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                        />
                                        <p className='text-red-500'>{errors.start_date?.message}</p>
                                    </div>

                                    <div>
                                        <label className="block text-sm font-medium text-gray-700">
                                            End Date
                                        </label>
                                        <input
                                            type="date"
                                            name="end_date"
                                            {...register("end_date")}
                                            className="block w-full rounded-md border-gray-300 px-3 py-1.5 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                        />
                                        <p className='text-red-500'>{errors.end_date?.message}</p>
                                    </div>


                                    <div className="mt-6 flex justify-end gap-2">
                                        <Button
                                            type="button"
                                            onClick={close}
                                            className="rounded-md bg-gray-200 px-4 py-2 text-sm font-medium text-gray-800 hover:bg-gray-300 focus:outline-none"
                                        >
                                            Cancel
                                        </Button>
                                        <Button
                                            type="submit"
                                            className="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-400"
                                        >
                                            Save
                                        </Button>
                                    </div>
                                </form>
                            </DialogPanel>
                        </div>
                    </div>
                </Dialog>
            )}
        </>
    );
}
